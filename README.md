...
| `attic clone [remote] [--mono] [--force]`                        | Restore an existing overlay on a new machine. With `--mono`, the remote may be omitted when this machine tracks exactly one. Refuses to clobber existing files without `--force`.                                                                                   |
...
| `attic doctor [--fix] [--force] [--push]`                        | Audit every overlay's label against its origin remote; report drift, `--fix` corrects it, `--push` publishes the corrected map. Also reports overlays stopped mid-rebase and overlays orphaned by a host history rewrite.                                           |
